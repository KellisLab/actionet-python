...
- **Optional GPU acceleration**: NVIDIA CUDA backend for SVD and kernel reduction (opt-in, Linux/WSL2)
...
### Building with GPU Support (NVIDIA CUDA)

GPU support is **opt-in** and requires an NVIDIA GPU (Ampere architecture or newer, compute capability ≥ 8.0). The default `pip install .` produces a CPU-only build. GPU support is currently available on Linux x86_64 and WSL2; it is never enabled on macOS.

#### Prerequisites

- CUDA Toolkit ≥ 12.2 installed (e.g. via `conda` or [NVIDIA's official installer](https://developer.nvidia.com/cuda-downloads))
- A supported GPU (NVIDIA Ampere / Ada / Hopper: e.g. A100, RTX 30xx/40xx, H100)
- All standard CPU prerequisites (see above)

#### Option 1: Conda + CUDA Toolkit (Easiest)

```bash
conda create -n actionet-gpu python=3.12
conda activate actionet-gpu

# Install CUDA Toolkit and build dependencies
conda install -c conda-forge cmake compilers cuda-toolkit cuda-nvcc numpy scipy

# Clone and build with GPU support enabled
git clone https://github.com/KellisLab/actionet-python.git
cd actionet-python
git submodule update --init --recursive

LIBACTIONET_ENABLE_NVIDIA_GPU=ON pip install -e .
```

#### Option 2: System CUDA Toolkit (Linux / WSL2)

```bash
# Point to your CUDA installation if not in the default location
export CUDAToolkit_ROOT=/usr/local/cuda-12.2   # adjust path as needed
...
#### GPU Build Environment Variables

| Variable | Purpose | Default |
|---|---|---|
| `LIBACTIONET_ENABLE_NVIDIA_GPU` | Set to `ON` to opt in to the CUDA build | `OFF` |
| `CUDAToolkit_ROOT` | Path to a CUDA ≥ 12.2 install | auto-detected |
| `CMAKE_CUDA_ARCHITECTURES` | Target GPU architectures | `80;86;89;90` (Ampere and newer) |

> **Note:** Do not set `CMAKE_CUDA_ARCHITECTURES` below `80`. The libactionet GPU backend requires Ampere or newer and will refuse pre-Ampere devices at runtime.

#### Verifying GPU Support

After installation, check whether the GPU backend is compiled in and available:

```python
import actionet._core as _core

compiled, _ = _core.is_nvidia_gpu_backend_compiled(), ""
available, reason = _core.can_use_nvidia_gpu(0)

print("GPU compiled in:", compiled)
print("GPU available:", available, reason if not available else "")
```

#### Using the GPU Backend

The execution policy is controlled by three keyword arguments on GPU-eligible functions (`reduce_kernel`, `run_svd`):

| kwarg | type | default | meaning |
|---|---|---|---|
| `compute_backend` | `"auto"` \| `"cpu"` \| `"gpu"` | `"auto"` | Backend selector. `"auto"` uses GPU when available. |
| `device_id` | `int >= 0` | `0` | CUDA device ordinal. |
| `allow_cpu_fallback` | `bool` | `True` | If `True`, a GPU request silently falls back to CPU when the GPU is unavailable. If `False`, raises `RuntimeError`. |
...
# Explicitly request GPU; fall back to CPU if unavailable
an.reduce_kernel(adata, n_components=50, compute_backend="gpu")

# Hard GPU requirement (raises if GPU is not available)
an.reduce_kernel(adata, n_components=50, compute_backend="gpu", allow_cpu_fallback=False)

# Explicit CPU (useful to compare results or benchmark)
an.reduce_kernel(adata, n_components=50, compute_backend="cpu")
```

The resolved backend is recorded in `adata.uns["action_params"]` for auditing and reproducibility.

#### Error Handling

GPU-related failures raise `actionet.GpuError` (or its subclasses `GpuUnavailableError`, `GpuRuntimeError`) which inherit from `RuntimeError`:
...
#### Troubleshooting GPU Builds

**`LIBACTIONET_ENABLE_NVIDIA_GPU` has no effect / CUDA not found:**
- Verify `nvcc` is on `PATH` or set `CUDAToolkit_ROOT` explicitly.
- Check `conda list | grep cuda` or `nvcc --version`.

**Unsupported GPU architecture error at runtime:**
- Ensure your GPU is Ampere or newer (compute capability ≥ 8.0).
- To target an older architecture explicitly (not recommended), set `CMAKE_CUDA_ARCHITECTURES` — but libactionet will still reject compute capability < 8.0 at runtime.

**Mixed OpenMP + CUDA runtime warnings:**
- With a conda CUDA install, set `MKL_THREADING_LAYER=GNU` if MKL is also present.
- Use `LIBACTIONET_OPENMP_RUNTIME=GNU` for consistency:
  `LIBACTIONET_ENABLE_NVIDIA_GPU=ON pip install -e . -C cmake.define.LIBACTIONET_OPENMP_RUNTIME=GNU`

**WSL2-specific notes:**
- Requires Windows 11 + WSL2 kernel ≥ 5.10.43 with NVIDIA CUDA on WSL driver.
- The Windows NVIDIA driver version determines the available CUDA version inside WSL2 — do **not** install a separate Linux NVIDIA driver inside WSL2.
- Use `nvidia-smi` inside WSL2 to confirm the GPU is visible before building.
