...
def run_actionet(
    ...
    ----------
    adata : AnnData
        Annotated data matrix (cells x features).
    ...
    layout_init_key : str or None, optional (default: None)
        Alternate key in ``adata.obsm`` used as the initial coordinate matrix for UMAP
        optimization. Can be any embedding in ``adata.obsm`` with >=2 columns (e.g. "action"). `None` defaults to "archetype_footprint" (generated by this function).
    ...
    Notes
    -----
    **Backed AnnData caveat:** When ``inplace=False``, ``adata.copy()`` is
    called, which fully materializes the data matrix into memory. If the input
    is a backed (HDF5-streamed) AnnData, this can trigger an unexpectedly
    large allocation.
    ...
    initial_coordinates = adata.obsm[layout_init_key or "archetype_footprint"].copy()
    ...
    if layout_3d:
        ...
        # Warm-start: xy from converged 2D; z from the init matrix's 3rd column
